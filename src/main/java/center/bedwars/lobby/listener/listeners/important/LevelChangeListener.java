...
public class LevelChangeListener implements Listener {
    ...
    public void onLevelChange(LevelChangeEvent event) {
        ...
        Bukkit.getScheduler().runTaskLater(Lobby.getINSTANCE(), () -> updateExperienceBar(player), 5L);
    }
    ...
    private void updateExperienceBar(Player player) {
        try {
            if (!AlonsoLevelsAPI.isLoaded(player.getUniqueId())) return;
            ...
            Lobby.getINSTANCE().getLogger().warning("Error updating experience bar for " + player.getName() + ": " + e.getMessage());
        }
    }
}